...
if (document.currentScript) {
  supportedByPositConfig = { ...supportedByPositConfig, ...document.currentScript.dataset }
}
...
function addSupportedByPositBadge (config) {
  ...
  // Determine whether we're on a Quarto website
  // If not, a pkgdown website is assumed
  const generator = document.querySelector('meta[name="generator"]')
  const isQuarto = generator && generator.content.toLowerCase().startsWith('quarto')

  let navbarContainerElement
  let darkSelector
  let defaultDisplay

  if (isQuarto) {
    navbarContainerElement = document.querySelector('.navbar-container.container-fluid') // Used to insert badge
    darkSelector = 'body.quarto-dark'
    defaultDisplay = 'none'
  } else {
    navbarContainerElement = document.querySelector('#navbar') // Used to insert badge
    darkSelector = 'html[data-bs-theme="dark"]'
    defaultDisplay = 'flex'
  }

  const navElement = document.querySelector('nav.navbar') // Used for determining height of badge
  ...
  ${darkSelector} #supported-by-posit { background-color: ${config.darkBg}; outline: 0px solid ${config.darkBg}; }
  ${darkSelector} #logo-supported-by { fill: ${config.darkFg}; }
  ${darkSelector} #logo-posit { fill: ${config.darkFg}; }
  ${darkSelector} #logo-lt { fill: ${config.darkFg}; }
  ${darkSelector} #logo-gt { fill: ${config.darkFg}; }

  #supported-by-posit:hover {
    background-color: color-mix(in hsl, ${config.lightBg}, black 5%);
    outline: 4px solid color-mix(in hsl, ${config.lightBg}, black 5%);
  }

  ${darkSelector} #supported-by-posit:hover {
    background-color: color-mix(in hsl, ${config.darkBg}, white 5%);
    outline: 4px solid color-mix(in hsl, ${config.darkBg}, white 5%);
  }
  `
  ...
  supportedByPositElement.href = `https://opensource.posit.co?utm_source=${encodeURIComponent(utmSource)}&utm_medium=referral&utm_content=supported-by-posit`
